...
### `hat.py`
An **endless Droste zoom** into a tiling of **"the hat"** — the aperiodic monotile discovered by Smith, Myers, Kaplan & Goodman-Strauss in 2023. It's a single 13-sided shape that tiles the plane but *never* repeats periodically. The view falls into the tiling forever while slowly rotating, a vivid plasma field flowing across everything like light through stained glass. The reflected "anti-hats" (the ~1-in-7 mirror-image tiles the tiling can't avoid) glow as contrasting accents.
...
| `ROT_SPEED` | rotation speed (radians/sec; `0` = no spin) |
